...
import com.sinelynx.grindingrobot.core.network.interceptor.AuthInterceptor
...
object NetworkModule {
    ...
    /** APK 文件响应必须直接流向下载器，不能先被调试拦截器读入内存。 */
    private fun isApkOtaDownload(request: Request): Boolean =
        request.method == "GET" && request.url.host == "apk.4digitalcenter.cloud" && request.url.port == 8443
    ...
    fun provideOkHttpClient(
        ...
        return OkHttpClient.Builder()
            ...
            .addInterceptor { chain ->
                ...
                if (isApkOtaDownload(request)) chain.proceed(request) else loggingInterceptor.intercept(chain)
            }
            .apply {
                if (BuildConfig.DEBUG) {
                    ...
                    addInterceptor { chain ->
                        val request = chain.request()
                        if (isApkOtaDownload(request)) chain.proceed(request) else chucker.intercept(chain)
                    }
                }
            }
            // 请求失败重试
