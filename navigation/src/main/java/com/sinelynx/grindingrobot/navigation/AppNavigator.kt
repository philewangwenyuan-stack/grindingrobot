...
import com.sinelynx.grindingrobot.navigation.routes.MainRoutes
...
class AppNavigator @Inject constructor() {
    companion object {
        /** 导航防重点击时间窗（毫秒），后续可按需调整。 */
        private const val NAVIGATION_DEBOUNCE_MS = 300L
    }

    private val _navigationEvents = MutableSharedFlow<NavigationEvent>()
    val navigationEvents: SharedFlow<NavigationEvent> = _navigationEvents.asSharedFlow()
    private val navigationDebounceLock = Any()
    private var lastNavigationEventElapsedMs: Long = 0L
    ...
    suspend fun navigateTo(route: Any, navOptions: NavOptions? = null) {
        if (!shouldDispatchNavigationEvent()) return
    ...
    private fun shouldDispatchNavigationEvent(): Boolean {
        val now = SystemClock.elapsedRealtime()
        synchronized(navigationDebounceLock) {
            if (now - lastNavigationEventElapsedMs < NAVIGATION_DEBOUNCE_MS) {
                return false
            }
            lastNavigationEventElapsedMs = now
            return true
        }
    }
}
...
fun NavController.handleNavigationEvent(event: NavigationEvent) {
    when (event) {
        is NavigationEvent.NavigateTo -> {
            // 统一“回首页”策略：
            // 若目标是 AppHome，优先回退到已存在的 AppHome（保留该页面，不新建）；
            // 仅当回退失败（栈中不存在）时才执行正常 navigate。
            when (event.route) {
                MainRoutes.Main -> {
                    this.navigate(MainRoutes.Main) {
                        launchSingleTop = true
                        restoreState = false
                        popUpTo(this@handleNavigationEvent.graph.id) {
                            inclusive = false
                            saveState = false
                        }
                    }
